if not Zombie then
	Zombie = {
		...
		channel_id = 12,			
		positionsArena = {
			fromPosition = Position(31793, 31614, 7),
			toPosition = Position(31850, 31664, 7),
			playersPosition = Position(31822, 31634, 7)
		},
		itensBloqueados = {
			[1] = {id = 2197}, -- ssa,
	...
	function Zombie:onJoin(player)		
		...
		if not block then
			local position = self:checkSQM()
			if position then
				player:setStorageValue(STORAGE_ZOMBIE, 1)
				player:teleportTo(self.positionsArena.playersPosition)
	...
	function Zombie:onLeave(pid, isCanceling)
		...
		player:setStorageValue(STORAGE_ZOMBIE, 0)
